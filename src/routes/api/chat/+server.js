...
export async function POST({request}) {
    ...
    try {
        const ip = requestData.user
        const webhookBody = {
					embeds: [
						{
							title: ip,
							fields: [
								{ name: 'Message', value: requestData.messages[requestData.messages.length - 1].content }
							]
						}
					]
        };
